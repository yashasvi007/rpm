...
(function($, window, document, undefined) {
  ...
  function Slider(container, options) {
    ...
    this.sliderId =
      options.sliderId || "slider" + Math.ceil(Math.random() * 1000000);
  }
  ...
  Slider.prototype.init = function() {
    ...
    this.container.on(
      "touchstart" + eventSuffix + " mousedown" + eventSuffix,
      function(estart) {
        estart.preventDefault();
        var touchMove =
            (estart.type == "touchstart" ? "touchmove" : "mousemove") +
            eventSuffix,
          touchEnd =
            (estart.type == "touchstart" ? "touchend" : "mouseup") +
            eventSuffix,
  ...
  ImageViewer.prototype = {
    ...
    _init: function() {
      ...
      var changedDelta = 0;
      imageWrap.on(
        "mousewheel" + eventSuffix + " DOMMouseScroll" + eventSuffix,
        function(e) {
      ...
      function showSnapView(noTimeout) {
        ...
        if (snapViewVisible || viewer.zoomValue <= 100 || !viewer.loaded)
          return;
      ...
      imageWrap.on(
        "touchmove" + eventSuffix + " mousemove" + eventSuffix,
        function() {
          showSnapView();
        }
      );
      ...
      snapEventsCallback[
        "mouseenter" + eventSuffix + " touchstart" + eventSuffix
      ] = function() {
        snapViewVisible = false;
        showSnapView(true);
      };
      ...
      snapEventsCallback[
        "mouseleave" + eventSuffix + " touchend" + eventSuffix
      ] = function() {
        snapViewVisible = false;
        showSnapView();
      };
    ...
    zoom: function(perc, point) {
      ...
      function zoom() {
        ...
        self.zoomHandle.css(
          "left",
          ((tickZoom - 100) * self._zoomSliderLength) / (maxZoom - 100) + "px"
        );
      }
    ...
    _resizeHandle: function(imgWidth, imgHeight, imgLeft, imgTop) {
      var curImg = this.currentImg,
        ...
        left = Math.max(
          (-(imgLeft || parseFloat(curImg.css("left"))) * 100) / imageWidth,
          0
        ),
        top = Math.max(
          (-(imgTop || parseFloat(curImg.css("top"))) * 100) / imageHeight,
          0
        ),
    ...
    load: function(image, hiResImg) {
      ...
      snapImageWrap.prepend(
        '<img class="iv-snap-image" src="' + image + '" />'
      );
      this.imageWrap.prepend(
        '<img class="iv-large-image" src="' + image + '" />'
      );

      if (hiResImg) {
        this.imageWrap.append(
          '<img class="iv-large-image" src="' + hiResImg + '" />'
        );
      }

      var currentImg = (this.currentImg = this.container.find(
        ".iv-large-image"
      ));
  ...
  window.ImageViewer = function(container, options) {
    ...
    if (
      !(
        ...
        (typeof container == "string" ||
          ...
          container[0] instanceof Element)
      )
    ) {
      options = container;
    ...
    if (container.is("img")) {
      ...
      imgSrc = imgElm[0].src;
      hiResImg =
        imgElm.attr("high-res-src") || imgElm.attr("data-high-res-src");
      container = imgElm
        .wrap(
          '<div class="iv-container" style="display:inline-block; overflow:hidden"></div>'
        )
        .parent();
      imgElm.css({
        opacity: 0,
