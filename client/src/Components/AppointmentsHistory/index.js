...
class AppointmentsHistory extends Component {
  ...
  myHandler = event => {
    ...
    if (scrolled === 1 && !end) {
      ...
      const { lastDate: startDate = moment() } = this.state;
      this.setState({ loading: true });
      fetchAppointmentsHistory(_id, startDate).then(resposne => {
        const { status, payload } = resposne;
        const { data: { historyIds = [] } = {} } = payload;
        console.log("status", status, historyIds.length);
  ...
  render() {
    ...
    return (
      <div className="history" id="history">
        <div className="title">My Appointments History</div>
        <div className="body" id="history-body">
