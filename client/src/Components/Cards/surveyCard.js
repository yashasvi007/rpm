...
const SurveyCard = ({
  ...
  const header = () => {
    return (
      <div
        className={`Inprogressnow flex align-items-center justify-content-space-between mb8 ${
          status === SURVEY_STATUS.COMPLETED ? "inprogress" : "completed"
        }`}
      >
        <div className={`fontsize12 dark ml10 `}>{status.toLowerCase()}</div>
  ...
  return (
    <div
      className="survey-card mr16 mb16 clickable"
