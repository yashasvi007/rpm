  ...
  renderEvents(eventsData, date) {
    ...
    return (
      <Fragment key={date}>
        ...
        <div className="card-wrapper">{options}</div>
      </Fragment>
    );
  }

  render() {
    const { selectedMenu /*currentDate*/ } = this.state;
    const { formatMessage } = this;
    ...
    return (
      <Fragment>
        {this.renderHeader()}
        {this.renderMenu()}
        {/* {this.pickRemindersByDate()} */}
        {/* {selectedMenu === OneTime && this.pickRemindersByDate} */}
        {/* {this.renderEventsDates(currentDate)} */}
